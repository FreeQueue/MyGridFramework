...
namespace OpenRA
{
	...
	public sealed class Actor : IEquatable<Actor> //, IDisposable
	{
		...
		// public bool IsDead => Disposed || (_health != null && _health.IsDead);
		//
		// public Vector2Int Location => OccupiesSpace.TopLeft;
		// public Vector2 CenterPosition => OccupiesSpace.CenterPosition;
		//
		// public WRot Orientation => _facing?.Orientation ?? WRot.None;

		/// <summary>Value used to represent an invalid token.</summary>
		public static readonly int InvalidConditionToken = -1;

		//private readonly IFacing _facing;
		private readonly INotifyBecomingIdle[] _becomingIdles;

		/// <summary>Cache of condition -> enabled state for quick evaluation of token counter conditions.</summary>
		private readonly Dictionary<string, int> _conditionCache = new Dictionary<string, int>();

		private readonly Dictionary<string, ConditionState> _conditionStates = new Dictionary<string, ConditionState>();

		/// <summary>Each granted condition receives a unique token that is used when revoking.</summary>
		private readonly Dictionary<int, string> _conditionTokens = new Dictionary<int, string>();
		private readonly IEnumerable<Vector2> _enabledTargetableWorldPositions;

		/// <summary>Read-only version of conditionCache that is passed to IConditionConsumers.</summary>
		private readonly IReadOnlyDictionary<string, int> _readOnlyConditionCache;
		private readonly INotifyIdle[] _tickIdles;
		...
		private int _nextConditionToken = 1;
		...
		//
		// 		foreach (TraitInfo traitInfo in Info.TraitsInConstructOrder())
		// 		{
		...
		// 			{ if (trait is IFacing t) _facing = t; }
		// 			{ if (trait is IHealth t) _health = t; }
		...
		// 			{ if (trait is IDefaultVisibility t) _defaultVisibility = t; }
		...
		// 		_visibilityModifiers = visibilityModifiersList.ToArray();
		// 		_becomingIdles = becomingIdlesList.ToArray();
		// 		_tickIdles = tickIdlesList.ToArray();
		// 		Targetables = targetablesList.ToArray();
		// 		var targetablePositions = targetablePositionsList.ToArray();
		// 		EnabledTargetablePositions = targetablePositions.Where(Exts.IsTraitEnabled);
		// 		_enabledTargetableWorldPositions = EnabledTargetablePositions.SelectMany(tp => tp.TargetablePositions(this));
		...
		// 	foreach (INotifyCreated t in TraitsImplementing<INotifyCreated>())
		// 		t.Created(this);
		//
		// 	var allObserverNotifiers = new HashSet<VariableObserverNotifier>();
		// 	foreach (IObservesVariables provider in TraitsImplementing<IObservesVariables>())
		// 	{
		// 		foreach (VariableObserver variableUser in provider.GetVariableObservers())
		// 		{
		// 			allObserverNotifiers.Add(variableUser.Notifier);
		// 			foreach (var variable in variableUser.Variables)
		// 			{
		// 				var cs = _conditionStates.GetOrAdd(variable);
		...
		// 				_conditionCache[variable] = cs.Tokens.Count;
		// 			}
		// 		}
		// 	}
		//
		// 	// Update all traits with their initial condition state
		// 	foreach (VariableObserverNotifier notify in allObserverNotifiers)
		// 		notify(this, _readOnlyConditionCache);
		//
		...
		// 	foreach (ICreationActivity ica in TraitsImplementing<ICreationActivity>())
		// 	{
		...
		//
		// 		Activity activity = ica.GetCreationActivity();
		// 		if (activity == null)
		// 			continue;
		//
		...
		// 	if (!wasIdle && IsIdle)
		// 	{
		// 		foreach (INotifyBecomingIdle n in _becomingIdles)
		...
		// 	}
		// 	else if (wasIdle)
		// 		foreach (INotifyIdle tickIdle in _tickIdles)
		...
		// 	foreach (IRenderModifier modifier in renderModifiers)
		// 		renderables = modifier.ModifyRender(this, wr, renderables);
		// 	return renderables;
		// }
		//
		// private IEnumerable<IRenderable> Renderables(WorldRenderer wr)
		// {
		// 	// PERF: Avoid LINQ.
		...
		// }
		//
		// private IEnumerable<Rectangle> Bounds(WorldRenderer wr)
		// {
		...
		// {
		// 	foreach (IMouseBounds mb in mouseBounds)
		// 	{
		...
		public void QueueActivity(Activity nextActivity) {
			if (!_created) {
				throw new InvalidOperationException(
					"An activity was queued before the actor was created. Queue it inside the INotifyCreated.Created callback instead.");
			}
		...
		// 			World.Remove(this);
		//
		// 		foreach (INotifyActorDisposing t in TraitsImplementing<INotifyActorDisposing>())
		// 			t.Disposing(this);
		//
		// 		World.TraitDict.RemoveActor(this);
		// 		Disposed = true;
		//
		// 		_luaInterface?.Value.OnActorDestroyed();
		// 	});
		// }
		//
		// public void ResolveOrder(Order order)
		// {
		// 	foreach (IResolveOrder r in resolveOrders)
		...
		// 	Generation++;
		//
		// 	foreach (INotifyOwnerChanged t in TraitsImplementing<INotifyOwnerChanged>())
		// 		t.OnOwnerChanged(this, oldOwner, newOwner);
		//
		// 	foreach (INotifyOwnerChanged t in World.WorldActor.TraitsImplementing<INotifyOwnerChanged>())
		// 		t.OnOwnerChanged(this, oldOwner, newOwner);
		//
		// 	if (wasInWorld)
		// 		World.Add(this);
		...
		//
		// 	return (_health == null) ? DamageState.Undamaged : _health.DamageState;
		// }
		//
		// public void InflictDamage(Actor attacker, Damage damage)
		// {
		// 	if (Disposed || _health == null)
		// 		return;
		//
		// 	_health.InflictDamage(this, attacker, damage, false);
		// }
		//
		...
		// 		return;
		//
		// 	_health.Kill(this, attacker, damageTypes);
		// }
		//
		// public bool CanBeViewedByPlayer(Player player)
		// {
		// 	// PERF: Avoid LINQ.
		// 	foreach (IVisibilityModifier visibilityModifier in _visibilityModifiers)
		// 		if (!visibilityModifier.IsVisible(this, player))
		// 			return false;
		//
		// 	return _defaultVisibility.IsVisible(this, player);
		// }
		//
		...
		// 		return _enabledTargetableWorldPositions;
		//
		// 	return new[] { CenterPosition };
		// }
		//
		// #region Conditions
		// private void UpdateConditionState(string condition, int token, bool isRevoke)
		// {
		...
		// {
		// 	return _conditionTokens.ContainsKey(token);
		// }
		//
		// #endregion
		//
		// #region Scripting interface
		// private Lazy<ScriptActorInterface> _luaInterface;
		// public void OnScriptBind(ScriptContext context)
		// {
		// 	if (_luaInterface == null)
		// 		_luaInterface = Exts.Lazy(() => new ScriptActorInterface(context, this));
		// }
		//
		// public LuaValue this[LuaRuntime runtime, LuaValue keyValue]
		// {
		// 	get => _luaInterface.Value[runtime, keyValue];
		// 	set => _luaInterface.Value[runtime, keyValue] = value;
		// }
		//
		...
		// {
		// 	return _luaInterface.Value.ContainsKey(name);
		// }
		//
		// #endregion
	}
