...
namespace Framework.Kits.YamlSerializeKits
{
	public static class FieldSaver
	{
		public static MiniYaml Save(object o, bool includePrivateByDefault = false) {
			...
			foreach (FieldLoadInfo info in FieldLoader.GetTypeLoadInfo(o.GetType(), includePrivateByDefault)) {
				if (info.Attribute.DictionaryFromYamlKey) {
					var dict = (IDictionary)info.Field.GetValue(o);
					foreach (object kvp in dict) {
						object key = ((DictionaryEntry)kvp).Key;
						object value = ((DictionaryEntry)kvp).Value;
		...
		public static MiniYaml SaveDifferences(object o, object from, bool includePrivateByDefault = false) {
			...
			IEnumerable<FieldLoadInfo> fields = FieldLoader.GetTypeLoadInfo(o.GetType(), includePrivateByDefault)
		...
		public static string FormatValue(object v) {
			...
			if (t.IsGenericType && (t.GetGenericTypeDefinition() == typeof(HashSet<>) ||
				...
				return ((IEnumerable)v).Cast<object>().Select(FormatValue).JoinWith(", ");
			}
			...
			if (t.IsGenericType && t.GetGenericTypeDefinition() == typeof(Dictionary<,>)) {
				...
				foreach (object kvp in dict) {
					...
					string formattedKey = FormatValue(key);
