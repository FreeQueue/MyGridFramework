...
namespace Framework.Kits.MiniYamlKits
{
	...
	public class MiniYaml
	{
		...
		public Dictionary<TKey, TElement> ToDictionary<TKey, TElement>(
			...
			foreach (MiniYamlNode y in Nodes) {
				TKey key = keySelector(y.Key);
				TElement element = elementSelector(y.Value);
				try {
					ret.Add(key, element);
				}
		...
		public MiniYaml(string value, List<MiniYamlNode> nodes = null) {
			Value = value;
			Nodes = nodes ?? new List<MiniYamlNode>();
		}

		private static List<MiniYamlNode> FromLines(
			IEnumerable<ReadOnlyMemory<char>> lines, string filename, bool discardCommentsAndWhitespace,
		...
		public static List<MiniYamlNode> FromFile(
			string path, bool discardCommentsAndWhitespace = true, Dictionary<string, string> stringPool = null
		) {
			return FromStream(File.OpenRead(path), path, discardCommentsAndWhitespace, stringPool);
		}

		public static List<MiniYamlNode> FromStream(
			Stream s, string fileName = "<no filename available>", bool discardCommentsAndWhitespace = true,
			Dictionary<string, string> stringPool = null
		) {
			return FromLines(s.ReadAllLinesAsMemory(), fileName, discardCommentsAndWhitespace, stringPool);
		}

		public static List<MiniYamlNode> FromString(
			string text, string fileName = "<no filename available>", bool discardCommentsAndWhitespace = true,
			Dictionary<string, string> stringPool = null
		) {
			return FromLines(text.Split(new[] { "\r\n", "\n" }, StringSplitOptions.None).Select(s => s.AsMemory()),
				fileName, discardCommentsAndWhitespace, stringPool);
		}
		...
		private static void MergeIntoResolved(
			MiniYamlNode overrideNode, List<MiniYamlNode> existingNodes,
			Dictionary<string, MiniYaml> tree, Dictionary<string, MiniYamlNode.SourceLocation> inherited
		) {
			MiniYamlNode existingNode = existingNodes.Find(n => n.Key == overrideNode.Key);
		...
		private static List<MiniYamlNode> ResolveInherits(
			MiniYaml node, Dictionary<string, MiniYaml> tree, Dictionary<string, MiniYamlNode.SourceLocation> inherited
		) {
			...
			foreach (MiniYamlNode n in node.Nodes) {
				if (n.Key == "Inherits" || n.Key.StartsWith("Inherits@", StringComparison.Ordinal)) {
					if (!tree.TryGetValue(n.Value.Value, out MiniYaml parent))
						throw new YamlException(
							$"{n.Location}: Parent type `{n.Value.Value}` not found");

					if (inherited.TryGetValue(n.Value.Value, out MiniYamlNode.SourceLocation value))
						throw new YamlException(
							$"{n.Location}: Parent type `{n.Value.Value}` was already inherited by this yaml tree at {value} (note: may be from a derived tree)");

					inherited.Add(n.Value.Value, n.Location);
					foreach (MiniYamlNode r in ResolveInherits(parent, tree, inherited))
		...
		private static MiniYaml MergePartial(MiniYaml existingNodes, MiniYaml overrideNodes) {
		...
		private static List<MiniYamlNode> MergePartial(
			List<MiniYamlNode> existingNodes, List<MiniYamlNode> overrideNodes
		) {
			...
			var existingDict = existingNodes.ToDictionaryWithConflictLog(x => x.Key, "MiniYaml.Merge", null,
				x => $"{x.Key} (at {x.Location})");
			var overrideDict = overrideNodes.ToDictionaryWithConflictLog(x => x.Key, "MiniYaml.Merge", null,
				x => $"{x.Key} (at {x.Location})");
			var allKeys = existingDict.Keys.Union(overrideDict.Keys);

			foreach (var key in allKeys) {
				existingDict.TryGetValue(key, out MiniYamlNode existingNode);
				overrideDict.TryGetValue(key, out MiniYamlNode overrideNode);

				MiniYamlNode.SourceLocation loc = overrideNode?.Location ?? default;
				var comment = (overrideNode ?? existingNode).Comment;
				MiniYamlNode merged = (existingNode == null || overrideNode == null)
					? overrideNode ?? existingNode
