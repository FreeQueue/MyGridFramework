...
namespace Framework.Kits.StreamKits
{
	...
	{
		private readonly Queue<byte> _data = new Queue<byte>(1024);
		private readonly Stream _baseStream;
		private bool _baseStreamEmpty;
		...
		public sealed override int ReadByte()
		{
			...
			while (!_baseStreamEmpty)
			{
				_baseStreamEmpty = BufferData(_baseStream, _data);
				if (_data.Count > 0)
					return _data.Dequeue();
			}
		...
		public sealed override int Read(byte[] buffer, int offset, int count)
		{
			...
			while (copied < count && !_baseStreamEmpty)
			{
				_baseStreamEmpty = BufferData(_baseStream, _data);
				ConsumeData(buffer, offset, count, ref copied);
			}
		...
		private void ConsumeData(byte[] buffer, int offset, int count, ref int copied)
		{
			while (copied < count && _data.Count > 0)
				buffer[offset + copied++] = _data.Dequeue();
		}
