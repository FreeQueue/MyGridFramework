...
{
	public static class SystemStreamExts
	{
		...
		public static string ReadAscii(this Stream s)
		{
			...
			try {
				...
				while ((read = sr.ReadBlock(buffer, offset, buffer.Length - offset)) != 0) {
					...
					while ((newlineIndex = Array.IndexOf(buffer, '\n', offset - read, read)) != -1) {
						...
						int afterNewlineIndex = newlineIndex + 1;
		...
		public static string ReadString(this Stream s, Encoding encoding, int maxLength)
		{
			...
			if (length > maxLength) {
				throw new InvalidOperationException(
					$"The length of the string ({length}) is longer than the maximum allowed ({maxLength}).");
			}
		...
		{
			byte[] bytes = !string.IsNullOrEmpty(text) ? encoding.GetBytes(text) : Array.Empty<byte>();
