	...
	partial class Util
	{
		...
		public static class Crypto
		{
			...
			public static byte[] GetXorBytes(byte[] bytes, int startIndex, int length, byte[] code) {
				...
				SArray.Copy(bytes, 0, results, 0, bytesLength);
			...
			private static readonly byte[] OIDHeader =
			...
			public static string EncodePEMPublicKey(RSAParameters parameters) {
				var data = SConvert.ToBase64String(EncodePublicKey(parameters));
				var output = new StringBuilder();
				output.AppendLine("-----BEGIN PUBLIC KEY-----");
				for (var i = 0; i < data.Length; i += 64)
					output.AppendLine(data.Substring(i, System.Math.Min(64, data.Length - i)));
			...
			public static RSAParameters DecodePEMPublicKey(string key) {
				try {
					...
					var lines = key.Split('\n');
					var data = SConvert.FromBase64String(lines.Skip(1).Take(lines.Length - 2).JoinWith(""));
			...
			private static byte[] EncodePublicKey(RSAParameters parameters) {
				...
				var modExpLength = TripletFullLength(parameters.Modulus.Length + 1) +
									TripletFullLength(parameters.Exponent.Length + 1);
			...
			private static void WriteTLVLength(BinaryWriter writer, int length) {
				if (length < 0x80) {
			...
			private static int ReadTLVLength(Stream s) {
				var length = s.ReadByte();
				if (length < 0x80)
					return length;

				var data = new byte[4];
				s.ReadBytes(data, 0, SMath.Min(length & 0x7F, 4));
				return BitConverter.ToInt32(data.ToArray(), 0);
			}

			private static int TripletFullLength(int dataLength) {
				if (dataLength < 0x80)
					return 2 + dataLength;
			...
			public static string DecryptString(RSAParameters parameters, string data) {
				try {
					...
					return Encoding.UTF8.GetString(rsa.Decrypt(SConvert.FromBase64String(data), false));
				}
				catch (Exception e) {
					Debug.LogFormat("Failed to decrypt string with exception: {0}", e);
					Debug.LogFormat("String decryption failed: {0}", e);
					return null;
				}
			}
			...
			public static string Sign(RSAParameters parameters, byte[] data) {
				try {
					...
					return SConvert.ToBase64String(rsa.SignHash(csp.ComputeHash(data),
						CryptoConfig.MapNameToOID("SHA1")));
				}
				catch (Exception e) {
					Debug.LogFormat("Failed to sign string with exception: {0}", e);
					Debug.LogFormat("String signing failed: {0}", e);
					return null;
				}
			}
			...
			public static bool VerifySignature(RSAParameters parameters, byte[] data, string signature) {
				try {
					...
					rsa.ImportParameters(parameters);
					using var csp = SHA1.Create();
					return rsa.VerifyHash(csp.ComputeHash(data), CryptoConfig.MapNameToOID("SHA1"),
						SConvert.FromBase64String(signature));
				}
				catch (Exception e) {
					Debug.LogFormat("Failed to verify signature with exception: {0}", e);
					Debug.LogFormat("Signature validation failed: {0}", e);
					return false;
				}
			}
