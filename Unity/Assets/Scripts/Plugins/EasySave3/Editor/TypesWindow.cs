...
namespace ES3Editor
{
	public class TypesWindow : SubWindow
	{
		...
		private void TypePane()
		{
			...
			TypeListItem typeListItem = types[selectedType];
			Type type = typeListItem.type;
		...
		private void DisplayFieldsOrProperties(bool showFields, bool showProperties)
		{
			...
			for(int i=0; i<fields.Length; i++)
			{
				ES3Reflection.ES3ReflectedMember field = fields[i];
		...
		private void TypeButton(int i)
		{
			...
			GUIStyle thisTypeButtonStyle = (i == selectedType) ? selectedTypeButtonStyle : typeButtonStyle;
		...
		private int LoadTypeIndex(string key)
		{
			...
			if(selectedTypeName != "")
			{
				Type type = ES3Reflection.GetType(selectedTypeName);
				if(type != null)
				{
		...
		private string GenerateWrites()
		{
			...
			for(int i=0; i<fields.Length; i++)
			{
				...
				ES3Type es3Type = ES3TypeMgr.GetES3Type(field.MemberType);
