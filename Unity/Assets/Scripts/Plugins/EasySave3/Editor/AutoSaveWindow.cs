...
namespace ES3Editor
{
	...
	public class AutoSaveWindow : SubWindow
	{
        ...
        public override void OnGUI()
		{
            ...
            using (var changeCheck = new EditorGUI.ChangeCheckScope())
            {
                ...
                using (var scrollView = new EditorGUILayout.ScrollViewScope(hierarchyScrollPosition, style.areaPadded))
                {
                    ...
                    foreach (HierarchyItem go in hierarchy)
                        if (go != null)
		...
		{
            if (mgr == null)
                foreach (ES3AutoSaveMgr thisMgr in Resources.FindObjectsOfTypeAll<ES3AutoSaveMgr>())
                    ...
                    foreach (Component component in components)
                    {
                        ...
                        using (var horizontalScope = new EditorGUILayout.HorizontalScope())
                        {
                            ...
                            if (newValue != saveComponent)
                            {
                                ...
                                {
                                    var so = new SerializedObject(autoSave);
                                    SerializedProperty prop = so.FindProperty("componentsToSave");
