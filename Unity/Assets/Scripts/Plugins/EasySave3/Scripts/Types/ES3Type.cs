	...
	public abstract class ES3Type
	{
		...
		protected object ReadProperties(ES3Reader reader, object obj)
		{
            ...
            foreach (string propertyName in reader.Properties)
			{
                ...
                {
                    ES3Type baseType = ES3TypeMgr.GetOrCreateES3Type(ES3Reflection.BaseType(obj.GetType()));
                    ...
                    else if(baseType.isCollection)
                    {
                        ...
                        {
                            MethodInfo method = baseType.type.GetMethod("Enqueue");
                            foreach (var item in loaded)
                                method.Invoke(obj, new object[] { item });
                        }
                        else if (type == typeof(ES3StackType))
                        {
                            MethodInfo method = baseType.type.GetMethod("Push");
                            foreach (var item in loaded)
                                method.Invoke(obj, new object[] { item });
                        }
                        else if (type == typeof(ES3HashSetType))
                        {
                            MethodInfo method = baseType.type.GetMethod("Add");
                            foreach (var item in loaded)
                                method.Invoke(obj, new object[] { item });
                        }
                    }
				...
				{
					ES3Type type = ES3TypeMgr.GetOrCreateES3Type(property.type);
