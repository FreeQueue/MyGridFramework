...
public class ES3
{
    ...
    public static void AppendRaw(byte[] bytes, ES3Settings settings)
    {
        ...
        using (Stream stream = ES3Stream.CreateStream(newSettings, ES3FileMode.Append))
            stream.Write(bytes, 0, bytes.Length);
    }
    ...
    internal static byte[] Serialize(object value, ES3Types.ES3Type type, ES3Settings settings = null)
    {
        ...
        using (var ms = new System.IO.MemoryStream())
        {
            using (Stream stream = ES3Stream.CreateStream(ms, settings, ES3FileMode.Write))
            {
    ...
    internal static object Deserialize(ES3Types.ES3Type type, byte[] bytes, ES3Settings settings = null)
    {
        ...
        using (var ms = new System.IO.MemoryStream(bytes, false))
            using (Stream stream = ES3Stream.CreateStream(ms, settings, ES3FileMode.Read))
    ...
    public static byte[] DecompressBytes(byte[] bytes)
    {
        using (var ms = new System.IO.MemoryStream(bytes))
        {
            ...
            {
                using (Stream input = ES3Stream.CreateStream(ms, settings, ES3FileMode.Read))
